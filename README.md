...
- OpenWrt 21.02 或更高版本
- WireGuard 支持 (`kmod-wireguard`, `wireguard-tools`)
...
A: 检查以下几点：
1. WireGuard内核模块是否加载：`lsmod | grep wireguard`
2. 接口是否正确创建：`ip link show warp`
3. 防火墙规则是否正确：`iptables -L -n | grep warp`
...
- [wgcf](https://github.com/ViRb3/wgcf) - WARP配置生成工具
